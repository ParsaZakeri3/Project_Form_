    ...
    public static class Validation
    {
        public static bool CheckDate(this string[] SplitDate)
        {
            if (SplitDate.Length == 3)
            {
                ...
                if (!resultmonth)
                {
                    resultmonth = SplitDate[1].Length == 1 && int.TryParse(SplitDate[1], out int num) ? num >= 1 && num <= 12 : false;
                }
                //check Day
                bool resultday = SplitDate[2].Length == 2 && int.TryParse(SplitDate[2], out int num_day) && num_day >= 1 && num_day <= 30 ? true : false;
                if (!resultday)
                {
                    resultday = SplitDate[1].Length == 1 && int.TryParse(SplitDate[2], out int num) ? num >= 1 && num <= 30 : false;
                }
                //check Date
                bool Date = resultYear && resultmonth && resultday ? true : false;
                return Date;
            }
            return false;
        }
        ...
        {
            bool result = int.TryParse(TryAgainForm, out int numtryagainform) && numtryagainform >= 1 && numtryagainform <= 2 ? true : false;
            return result;
        }
        ...
        public static string Checkphone(string check_number)
        {
            ...
            else if (check_number.StartsWith("+989"))
            {
                return check_number.Replace("+98", "0");
            }
            else if (check_number.StartsWith("9"))
            {
                return "0" + check_number;
            }
            {
                return "false";
            }
        ...
        {
            bool result = gender == "Male" || gender == "Famle" || gender == "Other" || gender == "male" || gender == "famle" || gender == "other" ? true : false;
            return result;
        }
        public static bool mustGoToMiliteryService(string gender, int age)
        {
            bool result = (gender == "male" || gender == "Male") && age >= 18 && age <= 50 ? true : false;
            return result;
        }
    }
}